...
// The receipt replaces this document on the same Window: stop timers first.
const lcSwap=swapTo;
swapTo=function(receipt){lcClose();return lcSwap(receipt)};
...
function lcMount(b){const c=$('#chat'),list=$('#msgs',c),hd=$('.hd',c),form=$('#compose',c),groups=D.groups||[];
 ...
 const input=$('#msg',form),sb=$('.send',form),st=document.createElement('div');
 st.className='sys lc-st';st.setAttribute('role','status');st.hidden=true;form.before(st);
 const say=(t,bad)=>{st.textContent=t||'';st.hidden=!t;st.classList.toggle('bad',!!bad);report()};
 const p=lcPane={id};
 ...
 p.live=LiveChat.mount({bridge:lcBridge,list,header:hd,target:{id,name:b.name,isGroup:!!b.group},
  ...
  renderItem:m=>lcItem(m,id),onBots:(bots,gs)=>lcBots(bots,gs,p,!!b.group),statusLine:say});
 if(!b.group){
  p.kit=ComposerKit.mount({bridge:lcBridge,form,input,sendButton:sb,bot:{id,name:b.name},status:say,
 ...
 p.live.refresh()}
// show.html's own row markup (thread()), one item at a time. Notices have no bot, so no orb.
function lcItem(m,id){if(typeof m.sys==='string'&&!m.bot)return m.sys?'<div class="sys">'+esc(m.sys)+'</div>':'';
 const bots=D.bots||[],who=m.sys&&!bots.some(x=>x.id===m.bot)?[{id:m.bot,name:m.sender||m.bot,color:'#888',shape:'blob'}]:[];
 return thread({bots:bots.concat(who),threads:{[id]:[m]}},id)}
