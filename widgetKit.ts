...
export interface FieldOptions {
  ...
  multiline?: boolean;
  /**
   * "date" renders the native calendar picker; the arg (and `value`) is an
   * ISO `YYYY-MM-DD` string. A date is never a free-text field the user has
   * to type — see BUILD.md R2-006. (todoist, 2026-09-05)
   */
  type?: "text" | "date";
}
...
export function vField(o: FieldOptions): Block {
  const key = esc(o.key);
  const type = o.type === "date" ? "date" : "text";
  const control = o.multiline
    ? `<textarea class="k-in" rows="2" data-k-key="${key}">${esc(o.value ?? "")}</textarea>`
    : `<input class="k-in" type="${type}" data-k-key="${key}" value="${esc(o.value ?? "")}">`;
  return block(
...
export interface SelectOption {
  /** What the tool receives — one of the arg's enum values. */
  value: string;
  label: string;
  /** Colours the field once chosen: good = green, warn = yellow, bad = red. */
  tone?: "good" | "warn" | "bad";
}
...
export interface SelectOptions {
  ...
  /** Pre-selected value; omitted or unknown → the placeholder row. */
  value?: string;
  /**
   * Text of a leading empty-value row ("Default"). With it, an arg the user
   * never spoke stays undefined instead of silently becoming the first option.
   */
  placeholder?: string;
}

/**
 * A dropdown for an enum argument, for confirmation cards. Same bridge
 * contract as {@link vField}: hydrated from `args` on init, posts
 * `voiceos:updateInput` on change. Priority/level args use this with the
 * low → good, medium → warn, high → bad tones (BUILD.md R2-006) — the chosen
 * option paints the field green / yellow / red. (todoist, 2026-09-05)
 */
export function vSelect(o: SelectOptions): Block {
  const key = esc(o.key);
  const rows: string[] = [];
  if (o.placeholder) {
    rows.push(`<option value=""${o.value ? "" : " selected"}>${esc(clip(o.placeholder, 32))}</option>`);
  }
  for (const opt of o.options) {
    const tone = opt.tone ? ` data-tone="${opt.tone}"` : "";
    const selected = opt.value === o.value ? " selected" : "";
    rows.push(`<option value="${esc(opt.value)}"${tone}${selected}>${esc(clip(opt.label, 32))}</option>`);
  }
  return block(
    `<label class="k-fld"><span class="k-fld-l">${esc(
      clip(o.label, 32),
    )}</span><select class="k-in k-sel" data-k-key="${key}">${rows.join("")}</select></label>`,
    54,
  );
}
...
:root{--ink-1:rgba(255,255,255,.95);--ink-2:rgba(255,255,255,.72);--ink-3:rgba(255,255,255,.5);--ink-4:rgba(255,255,255,.42);--line:rgba(255,255,255,.08);--fill-1:rgba(255,255,255,.06);--fill-2:rgba(255,255,255,.1);--track:rgba(255,255,255,.12);--good:#30d158;--warn:#ffd60a;--bad:#ff453a;--accent:#3987e5;--k-radius:26px;color-scheme:dark}
html[data-k-theme=light]{--ink-1:rgba(0,0,0,.92);--ink-2:rgba(0,0,0,.66);--ink-3:rgba(0,0,0,.46);--ink-4:rgba(0,0,0,.38);--line:rgba(0,0,0,.09);--fill-1:rgba(0,0,0,.04);--fill-2:rgba(0,0,0,.07);--track:rgba(0,0,0,.1);--warn:#b58a00;color-scheme:light;--accent:var(--accent-light,#2563c9)}
...
.k-in::-webkit-calendar-picker-indicator{opacity:.7;cursor:pointer}
.k-sel{cursor:pointer}
.k-sel[data-tone=good]{color:var(--good);border-color:var(--good)}
.k-sel[data-tone=warn]{color:var(--warn);border-color:var(--warn)}
.k-sel[data-tone=bad]{color:var(--bad);border-color:var(--bad)}
.k-sel option{color:var(--ink-1)}
.k-sel option[data-tone=good]{color:var(--good)}
.k-sel option[data-tone=warn]{color:var(--warn)}
.k-sel option[data-tone=bad]{color:var(--bad)}
...
/* A select paints itself with the tone of its chosen option (green / yellow /
   red for low / medium / high). Runs after hydration and on every change. */
var paintSel=function(s){
  if(!s||!s.classList||!s.classList.contains('k-sel'))return;
  var opt=s.options[s.selectedIndex];
  var t=opt&&opt.getAttribute('data-tone');
  if(t)s.setAttribute('data-tone',t);else s.removeAttribute('data-tone');
};
addEventListener('message',function(e){
  ...
  var args=m.args||{};
  /* Identifier dropdowns (a task list, a project, a folder) list what the user
     actually HAS. The static options in the markup are the fallback; when the
     init message carries data[<name>] as an array of strings or {value,label},
     the <select data-k-options="<name>"> is rebuilt from it before args land,
     so the spoken value still selects. Whether the host sends per-call data
     for a frozen confirmation is the host's side (logged for VoiceOS,
     google tasks 2026-09-06); the card is ready either way. */
  var data=m.data||{};
  var sels=document.querySelectorAll('select[data-k-options]');
  for(var s=0;s<sels.length;s++){
    var src=data[sels[s].getAttribute('data-k-options')];
    if(!Array.isArray(src)||!src.length)continue;
    var keep=sels[s].value;
    while(sels[s].options.length)sels[s].remove(0);
    for(var j=0;j<src.length;j++){
      var o=src[j],opt=document.createElement('option');
      opt.value=(o&&typeof o==='object')?String(o.value):String(o);
      opt.textContent=(o&&typeof o==='object')?String(o.label||o.value):String(o);
      sels[s].appendChild(opt);
    }
    if(keep)sels[s].value=keep;
  }
...
document.addEventListener('change',function(e){
  /* <select> fires input in Chromium, but change is the universal one — post
     once more; the host keeps the latest value per key. */
  var key=argKey(e.target);
  paintSel(e.target);
  if(key&&e.target.tagName==='SELECT')send({type:'voiceos:updateInput',key:key,value:e.target.value});
});
