...
const liveCard = screenCard(bot, { wsUrl: "wss://example.cursorvm.com/websockify?port_token=x", viewerUrl: "https://example.cursorvm.com/vnc.html" });
const live = html(liveCard);
const liveChars = glanceChars(liveCard);
if (liveChars > CAP) fail(`live screen card glance is ${liveChars} chars, over the ${CAP} cap`);
...
if (!live.includes('"stream":"wss://example.cursorvm.com/websockify?port_token=x"')) fail("live card args.stream is not the WebSocket url");
